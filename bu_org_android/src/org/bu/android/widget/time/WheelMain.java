...
public class WheelMain {
	...
	public WheelMain(View view) {
		super();
		this.view = view;
		dateUIType = DateUIType._DATE;
		setView(view);
	}

	public WheelMain(View view, DateUIType dateUIType) {
		super();
		this.view = view;
		this.dateUIType = dateUIType;
		setView(view);
	}
	...
	public void initDateTimePicker(int year, int month, int day, int h, int m) {
		...
		if (dateUIType.index == DateUIType._DATE.index) {// 如果是日期
			wv_hours.setVisibility(View.GONE);
			wv_mins.setVisibility(View.GONE);
		} else if (dateUIType.index == DateUIType._TIME.index) {// 如果是时间
		...
		} else if (dateUIType.index == DateUIType._TIME.index) {// 如果是日期
			textSize = (screenheight / 100) * 5;
		} else if (dateUIType.index == DateUIType._DATE_TIME.index) {// 如果是日期
	...
	public long getTimeL() {
		try {
			if (dateUIType.index == DateUIType._DATE.index) {// 如果是日期
				...
				return dateFormat.parse(getTime()).getTime();
			} else if (dateUIType.index == DateUIType._TIME.index) {// 如果是日期
	...
	public String getTime() {
		...
		if (dateUIType.index == DateUIType._DATE.index) {// 如果是日期
			...
			sb.append((wv_year.getCurrentItem() + START_YEAR)).append("-").append((wv_month.getCurrentItem() + 1)).append("-").append((wv_day.getCurrentItem() + 1)).append(" ")
					.append(wv_hours.getCurrentItem()).append(":").append(wv_mins.getCurrentItem());
		}
		return sb.toString();
	}
}
