...
public class BuDragExListView extends ExpandableListView {
	...
	public void setAdapter(BaseExpandableListAdapter mAdapter) {
		super.setAdapter(mAdapter);
		this.mAdapter = mAdapter;
	}

	private BuDragExView item_temp = null;
	...
	public BuDragExListView(Context context) {
		this(context, null);
	}

	public BuDragExListView(Context context, AttributeSet attr) {
		super(context, attr);
		stopDragging();
		mContext = context;
	...
	public boolean onTouchEvent(MotionEvent ev) {
		if (mDragView != null && mDragCurrentPostion != INVALID_POSITION && mDropViewListener != null) {
			...
			case MotionEvent.ACTION_UP:
				...
				// 数据交换
				int groupPosition = item_temp.getGroupPosition();
				if (mDragCurrentPostion >= groupPosition + 1 && mDragCurrentPostion < mAdapter.getChildrenCount(groupPosition) + groupPosition + 1) {
					mDropViewListener.drop(groupPosition, mDragStartPosition, mDragCurrentPostion);
				} else if (mDragCurrentPostion >= 0) {
					int whichgroup = 0;
					if (mDragCurrentPostion >= mDragStartPosition) {
						whichgroup = mDragCurrentPostion - mAdapter.getChildrenCount(groupPosition);
					} else {
						whichgroup = mDragCurrentPostion;
					}
					mDropViewListener.dropTgroup(groupPosition, mDragStartPosition, whichgroup);
				}
				break;
			case MotionEvent.ACTION_MOVE:
	...
	public interface DropViewListener {
		void drop(int groupPosition, int from, int to);

		void dropTgroup(int groupPosition, int from, int towhichgroup);
	}
