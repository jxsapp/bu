...
/**
 * ExpandableListView with pinned header using a fake ImageView. <b>NOTE</b>
 * that as the ImageView is fake, the ScrollBar maybe partially invisible.
 * 
		...
		if (mOnScrollListener == null) {
			mOnScrollListener = new OnScrollListener() {
				...
				public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
					...
					if (position == ExpandableListView.PACKED_POSITION_VALUE_NULL)
						return;
					int groupPosition = ExpandableListView.getPackedPositionGroup(position);

					if (groupPosition != mGroupPosition) {
						// If a new group comes to be the first visible one,
						// save the drawing cache
						...
						// If v is null, it appears that the group view is not
						// visible and so (maybe) we cannot retrieve the drawing
						// cache
						...
						// If next group is about to be the first one, scroll
						// the previous pinned header according
						// to the next group view's position
						long positionNext = mExpandableListView.getExpandableListPosition(firstVisibleItem + 1);
						if (positionNext == ExpandableListView.PACKED_POSITION_VALUE_NULL)
							return;
